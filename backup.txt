...
const walls = map.createLayer('walls', hutTileset, 0, 0);
    ...
    waterIndex.forEach(index => {
      map.setCollision(index, true, this.groundLayer);
  });
...
// Add collision callback with console logging
 ...
 this.physics.add.collider(this.protagonist, this.waterLayer, () => {
  console.log("Player collided with water layer");
});
...
map.setCollisionBetween(188, 188, true, waterLayer);
  ...
  update() {
    ...
    this.physics.collide(this.protagonist, this.walls, null, null, this);
    this.physics.collide(this.protagonist, this.waterLayer, () => {
      console.log("hit");
  }, null, this);
